...
const QuestionAnswer = () => {
    ...
    const [questionsWithAnswer , setQuestionsWithAnswer] = useState<{Oquestion: Question , questionreponses: QuestionReponse[]}[]>([]);
    useEffect(() => {
        fetch("http://localhost:8080/Questionannonce/getQuestionsByIdAnnonce/" + idannonce)
            .then((response) => response.json())
    ...
    const fetchDataForAllQuestions = async () => {
        try {
            const updatequestionswithanswer: { Oquestion: Question; questionreponses: QuestionReponse[] }[] = [];
            await Promise.all(
                changequestions.map(async (quest) => {
                    const response = await fetch("http://localhost:8080/Questionreponse/getByIdQuestion/" + quest.id);
                    const data = await response.json();
                    updatequestionswithanswer.push({ Oquestion: quest, questionreponses: data as QuestionReponse[] });
                })
            );
    ...
    const handleAnswerSelect = (question_id: number, reponse: string) => {
        ...
        const answerIndex = updatedQuestions[questionIndex].questionreponses.findIndex(
          (questionReponse) => questionReponse.reponse === reponse
        );
      
        if(updatedQuestions[questionIndex].questionreponses[answerIndex].change === undefined)
        {
            updatedQuestions[questionIndex].questionreponses[answerIndex].change = true;
        }else{
            updatedQuestions[questionIndex].questionreponses[
            answerIndex
            ].change = !updatedQuestions[questionIndex].questionreponses[answerIndex]
            .change;
        }
    ...
    const calculateScore = () => {
        ...
        for(const questionBlock of questionsWithAnswer)
        {
            ...
            {
                point_total += 1;
                if(questionreponse.change === undefined)questionreponse.change = false;
